...
interface SidebarProps {
  sidebarOpenDF: boolean;
  setSidebarOpenDF: (arg: boolean) => void;
}

const SidebarDF = ({ sidebarOpenDF, setSidebarOpenDF }: SidebarProps) => {
  ...
  useEffect(() => {
    const keyHandler = ({ keyCode }: KeyboardEvent) => {
      if (!sidebarOpenDF || keyCode !== 27) return;
      setSidebarOpenDF(false);
    };
  ...
  return (
    <aside
      ...
      className={`absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpenDF ? 'translate-x-0' : '-translate-x-full'
      }`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
      <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        ...
        <button
          ref={trigger}
          onClick={() => setSidebarOpenDF(!sidebarOpenDF)}
          aria-controls="sidebar"
          aria-expanded={sidebarOpenDF}
