...
import { getCourseByEmail, listCourse } from '../../services/CourseService/CourseService';
...
const TableCourse = () => {
  ...
  const fetchCourse = async (Email: string) => {
    try {
      ...
      if(token?.role.includes('ROLE_INTRUCTOR')) {
        const response = await getCourseByEmail(Email);
        formattedCourses = response.data; // Call with the Email parameter
      }
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
          {courses.map((course) => (
              <tr key={course.courseId} className="border-b border-gray-200 dark:border-strokedark">
                <td className="py-5 px-4 pl-9 text-black dark:text-white xl:pl-11">
                  <h5 className="font-medium">{course.courseName}</h5>
                </td>
                <td className="py-5 px-4 text-black dark:text-white">
                  <p>{course.updateDate}</p>
                </td>
