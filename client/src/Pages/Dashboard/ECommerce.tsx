    ...
    const fetchData = async () => {
      try {
        const [dashboardResponse] = await Promise.all([
          fetch(import.meta.env.VITE_SERVERHOST+'/api/v1/auth/dashboard/stats',
            {
              headers: {
        ...
        setData({
          totalProfit: dashboardResult.totalProfit,
          totalCategories: dashboardResult.totalCategoríes,
          totalUsers: dashboardResult.totalUsers,
          totalCourses: dashboardResult.totalCourses,
        });
      } catch (error) {
  ...
  return (
    <div>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 lg:grid-cols-3 xl:grid-cols-4">
        <CardDataStats title="Total Profit" total={`$${data.totalProfit}`}>
          <svg
            fill="#6fa6e2"
        ...
        <CardDataStats title="Total Courses" total={data.totalCourses}>
          <svg
            fill="#6fa6e2"
            width="64px"
            height="64px"
        ...
        <CardDataStats title="Total Categories" total={data.totalCategories}>
          <svg
            version="1.1"
            id="Layer_1"
            xmlns="http://www.w3.org/2000/svg"
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:gap-6 lg:gap-6 xl:gap-8">
        {/* ChartOne on its own row */}
        <div className="col-span-1">
          <ChartOne />
        </div>

        {/* ChartTrial and MapOne on the same row */}
        ...
        {/* RevenueByCategoryChart on its own row */}
        <div className="col-span-1">
          <RevenueByCategoryChart />
        </div>

        {/* Table and ChatCard */}
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:gap-6 xl:gap-8"></div>
      </div>
    </div>
  );
};
